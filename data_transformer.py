...
def z_score_norm(items: Sequence[int|float]) -> Sequence[float]:
    ...
    mu = mean(items)  # Compute mean    
    sigma = stdev(items)  # Compute standard deviation
    return [(item - mu) / sigma  for item in items]
...
def min_max_norm(items: Sequence[int|float]) -> Sequence[float]:
    ...
    if min_item == max_item: return [0.0 for _ in range(len(items))] 
    return [(item - min_item) / (max_item - min_item) for item in items]
...
def make_named_bins(items: Sequence[int|float], cut: str, names: Sequence[str]):
    ...
    bins = _find_bins(items=items, cut=cut, bin_count=len(names))
    return [names[i] for i in bins]
